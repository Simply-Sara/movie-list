...
function AddMediaForm({ onAdd, onCancel, currentUser, users }) {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <h3 className="text-lg font-semibold text-gray-800 mb-4">Add New Media</h3>
          ...
          <div className="p-2 space-y-2">
            {searchResults.map((result) => (
              <button
                ...
                className="w-full flex items-center gap-3 p-3 hover:bg-gray-50 rounded-lg transition text-left disabled:opacity-50"
              >
                {result.poster_path && (
                  <img
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="px-2 py-0.5 bg-indigo-100 text-indigo-800 text-xs font-medium rounded flex-shrink-0">
                      {getTypeLabel(result.type)}
                    </span>
                  </div>
                  {result.release_date && (
                    <p className="text-sm text-gray-600">{formatDate(result.release_date)}</p>
                  )}
                  {result.rating && (
                    <p className="text-sm text-gray-600">⭐ {result.rating.toFixed(1)}</p>
                  )}
                  {result.overview && (
      ...
      <div className="flex gap-3">
        <button
          type="button"
          onClick={onCancel}
          className="flex-1 bg-gray-200 text-gray-700 py-2 px-4 rounded-lg hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition"
        >
          Cancel
        </button>
      </div>
